...
export const guestbookCheckins = pgTable("guestbook_checkins", {
	id: uuid("id").default(sql`uuid_generate_v4()`).primaryKey().notNull(),
	guestId: uuid("guest_id").notNull().references(() => invitationGuests.id, { onDelete: "cascade" } ),
...
export const staffLogs = pgTable("staff_logs", {
	...
	guestId: uuid("guest_id").references(() => invitationGuests.id, { onDelete: "cascade" } ),
...
export const clients = pgTable("clients", {
	...
	passwordEncrypted: text("password_encrypted").notNull(),
	email: varchar("email", { length: 255 }),
	slug: varchar("slug", { length: 255 }).references(() => invitationContents.slug, { onDelete: "set null" } ),
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow(),
	updatedAt: timestamp("updated_at", { mode: 'string' }).defaultNow(),
...
(table) => {
	return {
		...
		idxClientsSlug: index("idx_clients_slug").on(table.slug),
		clientsUsernameKey: unique("clients_username_key").on(table.username),
		clientsSlugKey: unique("clients_slug_key").on(table.slug),
	}
});
...
export const greetingSections = pgTable("greeting_sections", {
	id: uuid("id").defaultRandom().primaryKey().notNull(),
...
export const loveStoryBlocks = pgTable("love_story_blocks", {
	id: uuid("id").defaultRandom().primaryKey().notNull(),
	registrationId: uuid("registration_id").notNull().references(() => weddingRegistrations.id, { onDelete: "cascade" } ),
