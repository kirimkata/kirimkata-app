...
export const weddingRegistrations = pgTable("wedding_registrations", {
	...
	eventType: varchar("event_type", { length: 50 }).default('islam').notNull(),
	weddingDate: date("wedding_date").notNull(),
	timezone: varchar("timezone", { length: 10 }).default('WIB'),
	...
	event1Title: varchar("event1_title", { length: 100 }).default('Akad Nikah'),
	...
	event2Title: varchar("event2_title", { length: 100 }).default('Resepsi'),
	...
	streamingButtonLabel: varchar("streaming_button_label", { length: 100 }).default('Watch Live'),
...
export const gallerySettings = pgTable("gallery_settings", {
	...
	mainTitle: varchar("main_title", { length: 255 }).default('Our Moments'),
	backgroundColor: varchar("background_color", { length: 50 }).default('#F5F5F0'),
...
export const loveStorySettings = pgTable("love_story_settings", {
	registrationId: uuid("registration_id").primaryKey().notNull().references(() => weddingRegistrations.id, { onDelete: "cascade" }),
	mainTitle: varchar("main_title", { length: 255 }).default('Our Love Story'),
...
export const weddingGiftSettings = pgTable("wedding_gift_settings", {
	registrationId: uuid("registration_id").primaryKey().notNull().references(() => weddingRegistrations.id, { onDelete: "cascade" }),
	title: varchar("title", { length: 255 }).default('Wedding Gift'),
	...
	buttonLabel: varchar("button_label", { length: 100 }).default('Kirim Hadiah'),
...
// Global bank catalog — managed via DB, logo URLs stored in R2.
// To add a new bank: INSERT a row (no code changes needed).
export const banks = pgTable("banks", {
	...
	(table) => {
		return {
			...
			banksNameKey: unique("banks_name_key").on(table.name),
		}
	});