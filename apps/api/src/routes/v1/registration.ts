 ...
 * Upsert wedding registration — create if not exists, update if exists.
 * Requires authentication. On create, client_id comes from JWT token.
 */
router.put('/:slug', clientAuthMiddleware, async (c) => {
    try {
        ...
        if (!registration) {
            // Auto-create registration on first save (upsert behaviour)
            const requiredForCreate = ['bride_name', 'bride_full_name', 'groom_name', 'groom_full_name', 'event1_date', 'event1_time'];
            ...
            try {
                ...
                console.error('Error compiling new invitation:', compileError);
            }
        ...
        // Update existing
        const updated = await weddingRegistrationRepo.update(c.env, registration.id, body);
        ...
        console.error('Error upserting wedding registration:', error);
        return c.json({
            error: error.message || 'Failed to save wedding registration'
        }, 500);
    }
});
