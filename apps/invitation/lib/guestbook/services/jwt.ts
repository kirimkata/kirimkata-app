...
import { JWTPayload, QRTokenPayload, ClientJWTPayload, StaffJWTPayload } from '../types';
...
export function generateClientToken(
  payload: Omit<ClientJWTPayload, 'exp' | 'iat' | 'type'>,
  expiresIn: string = '8h'
): string {
...
export function generateStaffToken(
  payload: Omit<StaffJWTPayload, 'exp' | 'iat' | 'type'>,
  expiresIn: string = '8h'
): string {
  const secret = getJWTSecret();
  const fullPayload = {
