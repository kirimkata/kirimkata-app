...
import { getBrideConfig, getGroomConfig, getWeddingDateConfig } from '@/config/textConfig';
import { getTransitionValuesForSection, getDefaultTransitionValues } from '@/config/animationConfig';
...
export default function Section0({
  ...
  const displayGuestName = guestName === '' ? '' : (guestName?.trim() ? guestName.trim() : 'Tamu Undangan');
  const shouldRenderGuestName = guestName !== '';

  // Gate opening animation state
  const [isGateOpen, setIsGateOpen] = useState(false);
  const [isGateFading, setIsGateFading] = useState(false);
  const [shouldRenderGates, setShouldRenderGates] = useState(true);
  ...
  }, [isLoadingComplete]); // Re-run when loading completes
    ...
    >
      ...
      >
        ...
        <div className="mt-4" style={{ marginTop: '30px' }}>
            ...
            style={
              {
                ...(section0Typography.guestInfo
                  ...
                  : {}),
                marginBottom: shouldRenderGuestName ? undefined : 0,
              }
            }
          >
            Kepada Yth. Bapak/Ibu/Saudara/i
          </p>
          {shouldRenderGuestName ? (
            <p
          ...
          {!shouldRenderGuestName ? <div style={{ height: '30px' }} /> : null}
          <p
            ...
            style={
              {
                ...(section0Typography.guestInfo
                  ? getTypographyStyle(section0Typography.guestInfo)
                  : {}),
                marginTop: shouldRenderGuestName ? undefined : 0,
              }
            }
          >
